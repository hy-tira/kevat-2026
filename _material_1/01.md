...
Algoritmi voidaan esittää niin, että sille annetaan tietty syöte (_input_) eli lähtötiedot, joiden perusteella se tuottaa halutun tuloksen (_output_). Pythonissa algoritmi voidaan toteuttaa funktiona, jolloin algoritmin syöte annetaan funktion parametreina ja algoritmin tulos on funktion palautusarvo.

Tarkastellaan esimerkkinä ongelmaa, jossa annettuna on lista lukuja ja halutaan laskea, moniko luku on parillinen. Esimerkiksi jos lista on `[5, 4, 1, 7, 9, 6]`, haluttu tulos on `2`, koska luvut `4` ja `6` ovat parillisia.
...
Seuraava pääohjelma testaa funktiota:
...
Muuttujassa `n` annetaan testissä käytettävän listan pituus. Funktio `random.seed` määrittää siemenluvun (tässä `1337`), jonka ansiosta satunnaisluvut muodostetaan aina samalla tavalla. Tästä on hyötyä, kun testi suoritetaan useita kertoja eri algoritmeille, jolloin listan sisältö on aina sama ja algoritmien vertailu on tasapuolinen. Tässä funktion `random.randint` avulla muodostetaan lista, jossa on `n` satunnaista lukua väliltä $$1 \dots 10^6$$.
...
Algoritmin tehokkuutta voidaan analysoida matemaattisesti laskemalla, montako askelta algoritmi suorittaa, kun sille annetaan tietyn kokoinen syöte. Usein voidaan ajatella, että jokainen koodissa oleva rivi vastaa yhtä algoritmin askelta.
...
Yllä olevassa koodissa koodin rivit on numeroitu, jotta pystymme viittaamaan riveihin algoritmin analysoinnissa.
...
Aikavaativuus ilmoitetaan usein muodossa $$O(\cdots)$$, jossa kolmen pisteen tilalla on muuttujan $$n$$ sisältävä kaava, joka antaa algoritmin askelten määrän suuruusluokan. Muuttuja $$n$$ kuvaa syötteen kokoa. Esimerkiksi jos syötteenä on lista, $$n$$ tarkoittaa listan kokoa, ja jos syötteenä on merkkijono, $$n$$ tarkoittaa merkkijonon pituutta.

Esimerkiksi kun algoritmi suorittaa vähintään $$2n+2$$ askelta ja enintään $$3n+2$$ askelta, sen aikavaativuus on $$O(n)$$. Tässä tapauksessa ja usein muutenkin aikavaativuus saadaan, kun askelten märän kaavasta otetaan huomioon vain kaavan nopeiten kasvava osa ja poistetaan vakiokertoimet.
...
### Aikavaativuus matemaattisesti

Tarkasti määriteltynä algoritmin aikavaativuus on $$O(f(n))$$, jos voidaan valita vakiot $$c$$ ja $$n_0$$ niin, että algoritmi suorittaa aina enintään $$c f(n)$$ askelta, kun $$n \ge n_0$$.

**Esimerkki 1**

Tarkastellaan algoritmia, joka suorittaa $$3n+2$$ askelta. Kun halutaan todistaa, että algoritmin aikavaativuus on $$O(n)$$, täytyy löytää vakiot $$c$$ ja $$n_0$$ niin, että $$3n+2 \le cn$$, kun $$n \ge n_0$$. Voimme valita vakiot $$c=5$$ ja $$n_0=1$$. Kun $$n \ge 1$$, pätee $$3n+2 \le 3n+2n = 5n$$ eli $$3n+2 \le 5n$$.

**Esimerkki 2**

Tarkastellaan algoritmia, joka suorittaa $$n^2+1$$ askelta. Tällaisen algoritmin aikavaativuus ei ole $$O(n)$$, koska emme voi valita vakioita $$c$$ ja $$n_0$$ vaaditulla tavalla. Jos aikavaativuus olisi $$O(n)$$, pitäisi pystyä valitsemaan vakiot niin, että $$n^2+1 \le cn$$ eli $$n^2 \le cn$$ eli $$n \le c$$. Tämä ei ole mahdollista, koska $$n$$ voi kasvaa rajatta mutta $$c$$ on vakio.
