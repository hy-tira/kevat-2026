...
def best_profit(prices):
    ...
    for sell_day in range(n):
        min_buy_price = min(prices[0:sell_day+1])
        best = max(best, prices[sell_day] - min_buy_price)
    return best
```

Koodi laskee muuttujaan `min_buy_price` osakkeen halvimman hinnan päivään `sell_day` mennessä. Tämä on toteutettu hakemalla pienin alkio `min`-funktiolla listan alkuosassa `prices[0:sell_day+1]`. Tämän jälkeen saamme laskettua kaavalla `prices[sell_day] - min_buy_price` voiton, kun myymme osakkeen päivänä `sell_day`.

Tämä on toimiva algoritmi ja siinä on vain yksi silmukka, mutta algoritmi ei ole vielä tehokas. Ongelmana on, että muuttujan `min_buy_price` laskeminen on hidasta silmukassa, koska `min`-funktio käy läpi listan `prices` alkuosan. Tämä vie aikaa $$O(n)$$, minkä vuoksi algoritmin aikavaativuus on edelleen $$O(n^2)$$.
...
def best_profit(prices):
    ...
    for sell_day in range(n):
        min_buy_price = min(min_buy_price, prices[sell_day])
        best = max(best, prices[sell_day] - min_buy_price)
    return best
```

Nyt muuttujaa `min_buy_price` ei lasketa tyhjästä silmukan joka kierroksella, vaan uusi arvo lasketaan tehokkaasti edellisen arvon perusteella. Tämän muutoksen ansiosta silmukan jokainen kierros vie aikaa $$O(1)$$, jolloin algoritmin aikavaativuus on $$O(n)$$ ja algoritmi toimii tehokkaasti.
...
Tässä tilanteessa mahdollisia tapoja on 12: (0,1), (0,4), (0,6), (0,7), (2,4), (2,6), (2,7), (3,4), (3,6), (3,7), (5,6) ja (5,7).
...
def count_ways(bits):
    ...
    for left_pos in range(n):
        for right_pos in range(left_pos + 1, n):
            if bits[left_pos] == '0' and bits[right_pos] == '1':
                result += 1
    return result
```
...
Mietitään, miten selviäisimme yhdellä silmukalla. Kuten osakkeen hinnan laskemisessa, tässäkin tehtävässä hyvä lähestymistapa on käsitellä jokaisessa kohdassa tehokkaasti kyseiseen kohtaan päättyvät ratkaisut. Tarkemmin voimme laskea kussakin bittijonon kohdassa tehokkaasti tavat, joissa oikea bitti 1 on kyseisessä kohdassa ja vasen bitti 0 on jossain aiemmassa kohdassa:
...
def count_splits(numbers):
    ...
    for pos in range(n - 1):
        left_sum = sum(numbers[0:pos+1])
        right_sum = sum(numbers[pos+1:])
